<template>
        <div class="flex flex-col items-center justify-center gap-5 h-full px-6.25">
            <div v-for="delivery in delivery_types" :key="delivery.type" :class="[{'shadow': selectedDeliveryType == type}, {'pr-5': selectedDeliveryType.length == 0 || selectedDeliveryType != delivery.type}, delivery.available ? '' : 'opacity-50 pointer-events-none' , 'flex border border-color-[#E9F0EB] rounded-3xl justify-between min:w-[523px] w-full']" @click="selectDeliveryType(delivery)">
                <div class="flex flex-col justify-between gap-10 py-10 sm:px-11 px-2">
                    <h6 class="text-4xl font-bold sm:min-w-[158px]">
                        {{alias[delivery.type].label}}
                    </h6>
                    <p class="sm:text-5xl text-[#78A1BB] font-bold text-3xl">{{delivery.price}}$</p>
                </div>
                <img :src="alias[delivery.type].image" :alt="delivery.type" width="100" height="100">
                <div v-if="selectedDeliveryType == delivery.type" class="flex items-center justify-center px-2 sm:px-5 rounded-r-3xl  bg-gradient-to-b bg-gradient-310 from-[#FFFFFF00] to-[#65B3E4]">
